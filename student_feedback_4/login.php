<?php
    ...
    if (isset($_POST['submit'])) {
        if (empty($_POST['login']) || empty($_POST['token'])) {
            ...
            $query = "SELECT fname, scode, availyear, speriod, pname from student_details where login=? AND token=? LIMIT 1";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("ss", $login, $token);
            $stmt->execute();
            $stmt->bind_result($fname, $scode, $availyear, $speriod, $pname);
            $stmt->store_result();
            if($stmt->fetch()){
